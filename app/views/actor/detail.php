 <main class="flex-grow-1 p-4">
   <!-- Actor Info Card (now styled like your reference containers) -->
   ...
   <!-- Movies Container (also styled like your reference) -->
   <div class="bg-secondary-subtle bg-opacity-25 border border-secondary rounded-2 p-4">
     ...
     <div class="d-flex overflow-auto gap-3 py-1" style="scrollbar-width: thin; scrollbar-color: #444 #000;">
       ...
       <?php if (isset($data['movies']) && count($data['movies']) !== 0) : ?>
         <?php foreach ($data['movies'] as $d) : ?>
           <a class="d-flex flex-column align-items-center text-decoration-none flex-shrink-0" href="<?= BASE_URL ?>/movie/index/<?= $d['movie_id'] ?>" style="width: 120px;">
             ...
             <?php endif ?>
             <div class="mt-2 text-center text-truncate text-secondary" style="font-size: 0.875rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
               <?= $d['title'] ?>
             </div>
           </a>
         <?php endforeach ?>
       ...
       <!-- Add more movie items here as needed -->
     </div>
   </div>
 </main>